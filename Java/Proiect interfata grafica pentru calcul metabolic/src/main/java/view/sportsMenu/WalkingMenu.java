...
public class WalkingMenu extends GenericMenu {
    private JLabel speedL = new JLabel("Walking speed:");
    ...
    public WalkingMenu() {
        super();
        super.setTitle("Walking calories calculator");
        ...
        getCalculate().addActionListener(e -> {
            try {
                ...
                walking = new Walking(Double.parseDouble(getDistanceTF().getText()), Double.parseDouble(getDurationTF().getText()));
                speedTF.setText((int) walking.getSpeedKph() + "");
                getBmrTF().setText(personValues.calc_bmr() + "");
                getExerciseTF().setText(walking.mets() + "");
                int c = (int) CaloriesBurnedUtil.determineCalories(personValues.getBmr(), walking.mets());
        ...
        getStore().addActionListener(e -> {
            if (done) {
                ...
                try {
                    ...
                    wrHistory.write("-------\n" + dtf.format(now) + "\n" + personValues.toString() + walking.toString() +
                            "Calories burned: " + c + "\n-------\n\n");
                    wrHistory.flush();
                    wrHistory.close();
