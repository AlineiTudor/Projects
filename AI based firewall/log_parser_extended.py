...
def extractFeatures(method,path_enc,body_enc,headers):
    ...
    braces2=path.count(")")+body.count(")")
    for word in badWords:
        badWords_count[word]+=path.count(word)+body.count(word)
        for header in headers:
            badWords_count[word]+=headers[header].count(word)+headers[header].count(word)

    return [method,path_enc.strip(),body_enc.strip(),single_q,double_q,dashes,braces,spaces,at_sign,braces2]+list(badWords_count.values())

#Open logFile and writing to csvFile
f=open(csv_log_file,'w')
c=csv.writer(f)
c.writerow(["method","path","body","single_q","double_q","dashes","braces","spaces","at_sign","braces2"]+badWords)
f.close()
